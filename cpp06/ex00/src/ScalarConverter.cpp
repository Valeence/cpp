...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/31 22:59:14 by vandre            #+#    #+#             */
/*   Updated: 2024/12/30 18:23:05 by vandre           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ScalarConverter &ScalarConverter::operator=(const ScalarConverter &cpy)
{
	if (this != &cpy)
	{
		...
		theChar = cpy.theChar;
		theInt = cpy.theInt;
		theFloat = cpy.theFloat;
		theDouble = cpy.theDouble;
	}
	return (*this);
}
...
{
	if (temp / 10 == 0 && toConvert.find('.') == std::string::npos) //check chiffre 0-9 unique sans point
		return (1);
	return (0);
}
...
int ScalarConverter::parseString(std::string toConvert)
{
	...
	if (temp == HUGE_VAL || temp == -HUGE_VAL)
		return (-1);
	if (checkDigit(temp, toConvert) == 1 || errno == ERANGE || errno == EDOM || errno == EILSEQ) //cheeck si la string est bien convertie
		return (-1);
